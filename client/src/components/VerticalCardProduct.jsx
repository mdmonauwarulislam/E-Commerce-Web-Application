...
const VerticalCardProduct = ({category, heading}) => {
    ...
    // Vertical Product Card Skeleton
    const VerticalProductSkeleton = () => (
        <div className='w-full min-w-[280px] md:min-w-[320px] max-w-[280px] md:max-w-[320px] bg-white rounded-lg shadow-md overflow-hidden animate-pulse'>
            <div className='bg-gradient-to-br from-gray-200 to-gray-300 h-48 p-4 flex justify-center items-center'></div>
        ...
        <div className='flex items-center gap-4 md:gap-6 overflow-x-scroll scrollbar-none transition-all' ref={scrollElement}>
            ...
            {loading ? (
                Array.from({ length: 6 }).map((_, index) => (
                    <VerticalProductSkeleton key={index} />
                ))
            ) : (
                data.map((product,index)=>{
                    const cartItem = getCartItem(product._id);
                    return(
                        <Link key={product?._id || `product-${index}` } to={"product/"+product?._id} className='w-full min-w-[280px] md:min-w-[320px] max-w-[280px] md:max-w-[320px] bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-all duration-300 group'>
                            <div className='bg-gradient-to-br from-gray-100 to-gray-200 h-48 p-4 flex justify-center items-center'>
                                <img src={product.productImage[0]} className='object-scale-down h-full group-hover:scale-110 transition-all duration-300 mix-blend-multiply'/>
                            </div>
                            <div className='p-4 grid gap-3'>
